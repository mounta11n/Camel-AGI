...
import { Button, Stack, Image, Form, Spinner, InputGroup,FormControl } from "react-bootstrap";
...
function AgentConvo() {
    ...
    const [keyAdded, setKeyAdded] = useState(true);
    ...
    //         // Set isLoggedIn to always be true.
    //     //     setIsLoggedIn(true)
    //     //     setUser({id:res.data.userId,image:res.data.image})
    //     //     if(res.data.key_added===null){
    //     //         setKeyAdded((prev)=>false)
    ...
    //         if(res.data.isLoggedIn === false){
    //             setAuthUrl(res.data.auth_url)
    //             setUser({id:res.data.userId,image:res.data.image}) // ist zu viel, habe ich selbst hinzugefügt debugging
    //         }else{
    //             setUser({id:res.data.userId,image:res.data.image})
    //             if(res.data.key_added===null){
    //                 setKeyAdded((prev)=>false)
        ...
        <>
            <div className="agent-convo-container vh-100 d-flex justify-content-center align-items-center">
                <div className="agent-convo-box px-3 py-4 p-md-5 text-white">
                                ...
                                {started?
                                    <>
                                        <Stack className="agent-scroll" gap={3}>
                                            ...
                                            {sessionRef.current!==0&&<Button className="align-self-end agent-share-btn" onClick={shareChat}><ShareIcon className="small-icon me-1"/><small>Teilen</small></Button>}
                                            <div className="end-chat"></div>
                                        </Stack>
                                        {finished?
                                            <h5><b>Aufgabe erledigt!</b></h5>
                                        :
                                        <>
                                            {turnRef.current<3?
                                                <Button className="continue-button mt-auto d-flex gap-2 justify-content-center align-items-center" disabled><Spinner size="sm"/>Wird geladen</Button>
                                                :<Button className="continue-button mt-auto" onClick={()=>startDiscussion(true)}>Weitermachen</Button>}
                                        </>}
                                    </>
                                :
                                    <>
                                        <h5 className="m-0">Beginnen Sie mit einer Aufgabe, um zu diskutieren</h5>
                                        <Form.Group className=" mt-4" >
                                            <Form.Label >In die Rolle des Ausbilders eintreten</Form.Label>
                                            <Form.Control className="agent-input" value={role1} placeholder="Börsenhändler" onChange={(e)=>setRole1(e.target.value)}/>
                                        </Form.Group>
                                        <Form.Group className=" " >
                                            <Form.Label >Geben Sie die Rolle des Assistenten ein.</Form.Label>
                                            <Form.Control className="agent-input" value={role2} placeholder="Python-Programmierer" onChange={(e)=>setRole2(e.target.value)}/>
                                        </Form.Group>
                                        <Form.Group className=" " >
                                            <Form.Label >Geben Sie Diskussionsthemen ein.</Form.Label>
                                            <Form.Control className="agent-input" value={task}  placeholder="Entwickeln Sie einen Handelsbot für den Aktienmarkt" onChange={(e)=>setTask(e.target.value)}/>
                                        </Form.Group>
										<Button className="continue-button mt-auto" onClick={()=>startDiscussion(true)}>Diskussion beginnen</Button>
                                    </>
                                
                                }
                            </>:
                            <Stack className="align-items-center" gap={3}>
                                <h6><b>Fügen Sie Ihren OpenAI-Schlüssel hinzu</b></h6>
                                <p><small>Holen Sie sich Ihren OpenAI-Schlüssel, indem Sie sich beim OpenAI-Dashboard anmelden</small></p>
                                <InputGroup >
                                    ...
                                    <Button variant="outline-secondary" className="chat-button text-dark fw-bold ps-md-3 pe-md-5 py-md-2" onClick={addKey} style={{color: 'white'}}>
                                    </Button>
                                </InputGroup>
                                {/* <small>Sehen Sie sich dieses Video an, um loszulegen</small>
                    ...
                    <Stack className="align-items-center" gap={4}>
                         <Image className="logo mt-5 pt-5" src={logoImage}/>
                            ...
                            <h5 className="">Erledigen Sie Ihre Aufgabe mit KI-Agenten</h5>
                            <Button className="option-buttons px-3 py-2 align-self-center" href={authUrl}>Melden Sie sich mit Google an</Button>
                ...
                <Stack className="align-items-center" gap={3}>
                    ...
                    <a className="discord-invite px-3 py-3 ms-3" target="_blank" href="https://discord.gg/A6EzvsKX4u"><DiscordIcon className="icon me-1"/>Discord</a>
                    <a className="discord-invite px-3 py-3 ms-3" target="_blank" href="https://github.com/SamurAIGPT/Camel-AutoGPT/"> Camel-AGI auf Github</a>
                </Stack>
                </Modal.Body>
            </Modal> 				 */}
            </div>
